...
void SetSocketStatus(char *socket_status) {
    /* 原来把 &user_config->socket_status[i]（char）强转成 int* 交给 sscanf 写，
     * 每个转换会写 4 字节：socket_status[6] 只有 6 字节，最后一写会越界覆盖到
     * 紧随其后的 user[0..2] —— 也就是每次从网页改开关状态，都会悄悄清掉
     * 童锁标志和头两个按键映射。必须先读到临时 int 数组再逐字节回填。 */
    int v[SOCKET_NUM] = {0};
    int got = sscanf(socket_status, "%d,%d,%d,%d,%d,%d",
    ...
    for (; i < SOCKET_NUM; i++) {
        if (i >= got) {
            /* 少给的字段保持原值，不要当成 0 全部关掉 */
            v[i] = user_config->socket_status[i];
        }
        UserRelaySet(i, v[i] ? 1 : 0);
 ...
 * on:开关 0:关 1:开 -1:切换
 */
void UserRelaySet(int i, char on) {
...
/*
 * KeyEventHandler 里会写 flash、起停 AP、重启 httpd，甚至直接复位系统。
 * 这些操作原来是在定时器回调线程里同步执行的：MiCO 的定时器回调跑在唯一的
 * timer 线程上，一旦阻塞，所有定时器（含按键扫描、LED、MQTT 上报定时器）都会
 * 停摆，重则与 WiFi 协议栈互相等待导致看门狗复位。这里改成投递到 worker 线程。
 */
static mico_worker_thread_t key_worker_thread;
static bool key_worker_ready = false;

static OSStatus KeyEventWorkerHandler(void *arg)
{
    int packed = (int) (intptr_t) arg;

    KeyEventHandler(packed >> 1, (packed & 1) ? true : false);
    return kNoErr;
}

static void KeyPostEvent(int num, boolean longPress)
{
    int packed;
    ...
    if (!key_worker_ready) {
        KeyEventHandler(num, longPress);
        return;
    }

    packed = (num << 1) | (longPress ? 1 : 0);
    if (mico_rtos_send_asynchronous_event(&key_worker_thread, KeyEventWorkerHandler,
                                          (void *) (intptr_t) packed) != kNoErr) {
        key_log("post key event failed");
    }
}
...
static void ClickEndTimeoutHandler(void *arg) {
    ...
    KeyPostEvent(click_count, false);
    click_count = 0;
}
...
void KeyInit(void) {
    if (!key_worker_ready) {
        if (mico_rtos_create_worker_thread(&key_worker_thread, MICO_APPLICATION_PRIORITY,
                                           0x1000, 5) != kNoErr) {
            key_log("create key worker failed, key events run inline");
        } else {
            key_worker_ready = true;
        }
    }
